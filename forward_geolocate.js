//  Server URL that will receive sensor data. See https://github.com/lupyuen/gcloud-wifi-geolocation.
...
function pushSensorData(values, callback) {
  //  Push the sensor data to the Google Cloud AppEngine Server running gcloud-wifi-geolocation.
  ...
  httpRequest({
    ...
    if (err) { console.log('push error'); console.error(err); return callback(err); }
...
function trigger(params, callback) {
  ...
  //  If this is a valid geolocation request, forward to geolocate cloud function without waiting for it to complete.
  if (ssid0 && rssi0) {
  ...
  //  Otherwise push the sensor data to the external server without waiting for it to complete.
